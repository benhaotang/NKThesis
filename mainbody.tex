...
模板在2021年11月2日更新后在CTEX2.7和Texlive2021(Mac\TeX 2021)下测试\XeLaTeX 和PDF\LaTeX 通过， MiKTeX无法编译成功，Overleaf缺少对应的CJK字体也无法编译成功，请确保系统中安装了simHei（黑体）、simKai（楷体）、simSun（宋体）才能正常显示，请注意编译后要使用bibtex重新编译以更新目录和引用（如果使用WinEdt，建议安装LaTeXify插件简化此过程，其他编译器也有类似方法）。对于其它 TeX 发行版可能需要做个别修改。本模板改编自孙老师（http://222.30.48.141/$\sim$sunwch）的版本，格式参照南开大学2019年博士硕士毕业论文格式+2022年本科生毕业论文格式要求\footnote{220325 修正尾注}。
...
\hline
编译方式 & 调用的宏包\\ \hline
\XeLaTeX & xeCJK, CJKnumb, graphicx, mathptmx, xifthen, amsmath \\ \hline
[PDF]\LaTeX & CJK, CJKnumb, uniGBK, graphicx, mathptmx, xifthen, amsmath \\
\hline
\end{tabular}
\end{center}
...
参考文献引用:
\cite{book1,book2,book3,book4,book5,book6,book7}
文献\upcite{thesis2,patent1,patent2}\cite{pc1,pc2,pc3,report1,report2}
\cite[Theorem 2.1]{thesis1}

本模板中目前所有文献都填在了nkthesis.bib文件中\cite{patent3,paper1,paper2,paper3,paper4,paper5,paper6,paper7,paper8}，可以参考其中的entry来填写\cite{ele1,ele2,ele3,thesis2}
...
目前已经有很多优秀的\LaTeX\ 作图宏包，如 pgf/Tikz 和 pstricks，两者都具有强大的作图能力。以下图\ref{fig:shiyi}是一个Tikz作图的例子：
